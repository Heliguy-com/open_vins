...
# variables
################################################################################
VERSION=$(cat ros1_pkg/control/control | grep "Version" | cut -d' ' -f 2)
PACKAGE=$(cat ros1_pkg/control/control | grep "Package" | cut -d' ' -f 2)
IPK_NAME=${PACKAGE}_${VERSION}.ipk


DATA_DIR=ros1_pkg/data
CONTROL_DIR=ros1_pkg/control
IPK_DIR=ros1_pkg/IPK
DEB_DIR=ros1_pkg/DEB
...
# Cheeky way of including the voxl-opencv-3.4.2 libraries
# No work when there are diff versions of libraries already in /usr/lib64

# sudo mkdir -p ${DATA_DIR}/usr/lib64
# sudo cp /usr/lib64/libopencv_*.so.3.4* ${DATA_DIR}/usr/lib64
...
if $MAKE_DEB; then
    ...
    DEB_NAME=${PACKAGE}_custom_arm64.deb
